...
import { BarChart2, TrendingUp, Globe, Users, Loader2, AlertCircle, LayoutDashboard, UserPlus, Zap } from 'lucide-react'
import { getAIInsights } from '../api.js'

const SEVERITY_STYLES = {
  high:   { bar: 'bg-rose-500',   text: 'text-rose-400',   bg: 'bg-rose-500/10   border-rose-500/20',   label: '🔥 Trending',   icon: AlertCircle },
  medium: { bar: 'bg-amber-500',  text: 'text-amber-400',  bg: 'bg-amber-500/10  border-amber-500/20',  label: '📈 Rising',     icon: TrendingUp  },
...
export default function AIInsights({ session }) {
  ...
  useEffect(() => {
    getAIInsights()
      .then(r => { if (r.success) setInsights(r.data || r.insights || []) })
      .catch(() => {})
      .finally(() => setLoading(false))
  }, [])

  const STATS = [
    { icon: TrendingUp, label: 'Indian Students Abroad',  value: '1.3M+',   sub: 'In 2025–26'          },
    { icon: Globe,      label: 'Canada Acceptance Rate',  value: '61%',      sub: 'For MS programs'     },
    { icon: Users,      label: 'Avg Scholarships/Year',   value: '₹8.2L',    sub: 'Our students receive' },
    { icon: BarChart2,  label: 'Visa Success Rate',       value: '94%',      sub: 'GlobalPath average'  },
  ]
    ...
    <section id="insights" className="py-24 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          >
            <BarChart2 className="w-3.5 h-3.5" /> Study Abroad Intelligence
          </motion.div>
          <motion.h2
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            Real-time data on visa acceptance rates, admission trends, scholarship opportunities, and popular destinations — updated regularly.
          </motion.p>
        </div>
        ...
        >
          <p className="text-white/40 text-sm mb-4">
            {session
              ? 'View your personalised study abroad progress in your dashboard.'
              : 'Get personalised insights after booking your free consultation.'}
          </p>
          {session ? (
            ...
            <Link to="/signup" className="btn-brand inline-flex">
              Start Your Journey <UserPlus className="w-4 h-4 ml-1" />
            </Link>
          )}
        </motion.div>
      </div>
